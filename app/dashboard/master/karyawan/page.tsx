...
const EMPTY_FORM: KaryawanInput = { kode: '', nama: '', jabatan: '' };

const TABLE_COLS = [
  { key: 'nama',    label: 'NAMA KARYAWAN', cls: 'min-w-64' },
  { key: 'kode',    label: 'KODE',          cls: 'min-w-36' },
  { key: 'jabatan', label: 'JABATAN',        cls: 'min-w-48' },
];
...
interface ModalProps {
  ...
  onClose: () => void;
  onSave: (data: KaryawanInput) => Promise<void>;
  saving: boolean;
}

function KaryawanModal({ mode, initialData, onClose, onSave, saving }: ModalProps) {
  const [form, setForm] = useState<KaryawanInput>(initialData);

  function set<K extends keyof KaryawanInput>(field: K, value: KaryawanInput[K]) {
    setForm((prev) => ({ ...prev, [field]: value }));
  }

  const inp = 'border border-gray-300 rounded px-2 py-1 text-xs w-full focus:outline-none focus:ring-1 focus:ring-teal-400 bg-white';
  const lbl = 'block text-xs text-gray-600 mb-0.5';

  const title =
    mode === 'edit'
      ? `Edit Karyawan  Kode : ${initialData.kode}, Nama : ${initialData.nama}`
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-white rounded shadow-2xl w-96 flex flex-col border border-gray-300">
        {/* Title */}
        <div className="bg-teal-100 border-b border-teal-300 px-4 py-2 rounded-t">
        ...
        <form onSubmit={handleSubmit} className="flex-1">
          <div className="p-4">
            <fieldset className="border border-gray-300 rounded px-3 pb-3 pt-1">
              <legend className="text-xs font-semibold text-gray-600 px-1">-Identitas Karyawan-</legend>
...
export default function KaryawanPage() {
  ...
  const [filtered, setFiltered] = useState<IKaryawan[]>([]);
  const [selected, setSelected] = useState<IKaryawan | null>(null);
  ...
  async function handleSave(formData: KaryawanInput) {
    setSaving(true);
    setError('');
    try {
      const url = modalMode === 'edit' && selected ? `/api/karyawan/${selected._id}` : '/api/karyawan';
  ...
  async function handleDelete() {
    if (!selected) return;
    if (!confirm(`Hapus karyawan "${selected.nama}"?\n\nTindakan ini tidak dapat dibatalkan.`)) return;
    setError('');
    try {
      const res = await fetch(`/api/karyawan/${selected._id}`, { method: 'DELETE' });
  ...
  const initialFormData: KaryawanInput =
    modalMode === 'edit' && selected
      ? { kode: selected.kode, nama: selected.nama, jabatan: selected.jabatan }
      : EMPTY_FORM;

  return (
    <div
      className="flex flex-col rounded-lg overflow-hidden shadow-md border border-teal-200 bg-white"
      style={{ height: 'calc(100vh - 185px)' }}
    >
      {/* Title bar */}
      <div className="bg-teal-700 text-white px-4 py-2 font-bold text-sm shrink-0">
      ...
      <div className="px-3 py-2 border-b border-gray-200 flex items-center gap-3 shrink-0 bg-gray-50">
        ...
        <input
          ...
          placeholder="Nama, kode, atau jabatan..."
          className="border border-gray-300 rounded px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-teal-400 w-72"
        />
        <span className="text-xs text-gray-400">{filtered.length} karyawan</span>
        {error && (
        ...
        <table className="w-full text-xs border-collapse" style={{ minWidth: '400px' }}>
          <thead className="sticky top-0 z-10">
            <tr className="bg-teal-600 text-white">
              <th className="w-5 border-r border-teal-500 px-1" />
              {TABLE_COLS.map((c) => (
                <th
                  key={c.key}
                  className={`${c.cls} px-2 py-2 font-semibold border-r border-teal-500 whitespace-nowrap text-left last:border-r-0`}
                >
                  {c.label}
                </th>
              ))}
            ...
            {loading ? (
              <tr>
                <td colSpan={TABLE_COLS.length + 1} className="text-center py-12 text-gray-400">
                  <div className="flex flex-col items-center gap-2">
                    <svg className="animate-spin w-6 h-6 text-teal-400" fill="none" viewBox="0 0 24 24">
              ...
              <tr>
                <td colSpan={TABLE_COLS.length + 1} className="text-center py-12 text-gray-400 italic">
                  {search ? 'Tidak ada karyawan yang sesuai pencarian.' : 'Belum ada data karyawan. Klik Tambah untuk menambahkan.'}
                </td>
              </tr>
            ) : (
              filtered.map((k, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-2 py-1 text-gray-600">{k.jabatan}</td>
                  </tr>
                );
              })
            )}
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-t border-gray-200 bg-gray-50 shrink-0">
        ...
        {selected && (
          <span className="ml-2 text-xs text-teal-700 bg-teal-50 border border-teal-200 px-2 py-1 rounded">
