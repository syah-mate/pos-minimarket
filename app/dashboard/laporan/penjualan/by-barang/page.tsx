...
/** Catatan kaki, mengikuti laporan cetak lama. */
const CATATAN = 'harga jual sudah termasuk tax, dan beban credit card';
...
/** Satu baris laporan: gabungan item dengan kode, HPP, dan harga jual yang sama. */
...
const COLS = [
  { key: 'no', label: 'No.', w: 9, align: 'left' as const },
  { key: 'kode', label: 'Kode Barang', w: 28, align: 'left' as const },
  { key: 'nama', label: 'Nama Barang', w: 61, align: 'left' as const },
  { key: 'hpp', label: 'HPP', w: 21, align: 'right' as const },
  { key: 'harga', label: 'Harga Jual', w: 21, align: 'right' as const },
  { key: 'diskon', label: 'Diskon', w: 14, align: 'right' as const },
  { key: 'qty', label: 'QTY', w: 11, align: 'right' as const },
  { key: 'jumlah', label: 'JUMLAH', w: 25, align: 'right' as const },
];
...
export default function LaporanPenjualanByBarangPage() {
  ...
  const buildPdf = useCallback(
    (rows: BarisBarang[]) => {
      ...
      // ── Baris TOTAL: sel kiri menyatu, QTY & JUMLAH di kolomnya sendiri ──
      ...
      pdf.text(fmtNum(totalQty), COL_X[qtyIdx] + COLS[qtyIdx].w - PAD, y + HEAD_H - 2.2, { align: 'right' });
      pdf.text(fmtNum(totalJumlah), MARGIN_R - PAD, y + HEAD_H - 2.2, { align: 'right' });

      // ── Catatan kaki di bawah baris total ──
      pdf.setFont('helvetica', 'italic');
      pdf.setFontSize(7);
      pdf.setTextColor(60, 60, 60);
      pdf.text(CATATAN, MARGIN_L, y + HEAD_H + 4);
  ...
  const handleTampilkan = useCallback(async () => {
    ...
    try {
      ...
      // barang dan HPP — master barang diambil sekali lalu dipetakan.
      ...
      // Item digabung per kombinasi kode + HPP + harga jual, persis seperti
      // laporan lama: satu barang bisa muncul beberapa kali kalau dijual pada
      // harga yang berbeda.
      ...
      for (const t of transaksi) {
        ...
        for (const it of t.items ?? []) {
          ...
          const kode = b?.kode ?? it.barangId;
          // HPP dari harga beli master (rata-rata bergerak), per satuan jual.
          const hpp = Number(b?.hargaBeli) || 0;
          const harga = Number(it.harga) || 0;
          const key = `${kode}|${hpp}|${harga}`;
          const ada = grup.get(key);
          if (ada) {
            ada.qty += Number(it.qty) || 0;
            ada.diskon += Number(it.discRp) || 0;
            ada.jumlah += Number(it.subtotal) || 0;
          } else {
            grup.set(key, {
              kode,
  ...
  return (
    <div className="h-full grid grid-cols-1 lg:grid-cols-[300px_minmax(0,1fr)] xl:grid-cols-[320px_minmax(0,1fr)] gap-4 items-start lg:items-stretch overflow-y-auto lg:overflow-hidden">
      ...
      <div className="bg-white rounded-xl shadow p-5 self-start">
        <h2 className="text-base font-bold text-gray-800 mb-4">Laporan Penjualan by Barang</h2>
      ...
      <div className="bg-white rounded-xl shadow overflow-hidden min-w-0 h-full min-h-160">
        {pdfUrl ? (
          ...
          <iframe
            ...
            title="Laporan Penjualan by Barang"
          />
        ) : (
