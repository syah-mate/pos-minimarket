...
interface BayarHutangDoc {
  _id: string; refNo: string; tanggal: string;
  supplierId: string; supplierKode: string; supplierNama: string; supplierAlamat: string;
  kasId: string; kasKode: string; kasNama: string;
  keterangan: string; operator: string;
  items: Omit<ItemRow, '_key' | 'noHutangInput'>[];
  totalBayar: number;
}
...
function emptyRow(): ItemRow {
  return {
    ...
    transaksiBeliId: '', noHutangInput: '', noHutang: '', tglHutang: '',
    jmlHutang: 0, returnAmount: 0, angsuran: 0,
  };
}
function ensureEmptyLast(rows: ItemRow[]) {
  if (rows.length === 0 || rows[rows.length - 1].transaksiBeliId !== '') return [...rows, emptyRow()];
  return rows;
}

// ─── Supplier Picker ──────────────────────────────────────────────────────────

function SupplierPicker({ onSelect, onClose }: {
  onSelect: (s: SupplierOption) => void; onClose: () => void;
}) {
        ...
        <div className="overflow-auto flex-1">
          <table className="w-full text-xs border-collapse">
            <thead className="bg-red-600 text-white sticky top-0">
              <tr>{['KODE','NAMA','ALAMAT','SALDO HUTANG'].map(h =>
            ...
            <tbody>
              {filtered.map((s, i) => (
                <tr key={s._id} onClick={() => onSelect(s)}
                  ...
                  <td className="px-2 py-1 border border-gray-200 text-right">{fmt(s.saldoHutang)}</td>
                </tr>
              ))}
...
function HutangPicker({ supplierId, q: initialQ = '', onSelect, onClose }: {
  supplierId: string;
  q?: string;
  onSelect: (h: HutangOption) => void;
  onClose: () => void;
}) {
  const [q, setQ] = useState(initialQ);
  const [list, setList] = useState<HutangOption[]>([]);
  const [loading, setLoading] = useState(false);
  const inputRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    inputRef.current?.focus();
    if (supplierId) fetchHutang();
  }, [supplierId]);
  ...
  async function fetchHutang() {
    ...
    try {
      ...
      const result: HutangOption[] = beliData.map((b) => ({
        ...
        hutang: b.hutang,
        lunas: b.lunas,
        returnAmount: returnByRef[b.refNo] || 0,
      }));
      setList(result);
    } finally {
      setLoading(false);
  ...
  const filtered = list.filter(h =>
    h.refNo.toLowerCase().includes(q.toLowerCase())
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded shadow-xl flex flex-col" style={{ width: '70vw', maxHeight: '70vh' }}>
        <div className="bg-red-700 text-white px-4 py-2 font-bold text-sm flex justify-between shrink-0">
          <span>PILIH HUTANG PEMBELIAN</span>
          <button onClick={onClose} className="hover:text-red-300">✕</button>
        </div>
        <div className="p-2 border-b shrink-0">
          <input ref={inputRef} value={q} onChange={e => setQ(e.target.value)}
            placeholder="Cari no. hutang..." className="border rounded px-2 py-1 text-xs w-full" />
        </div>
        <div className="overflow-auto flex-1">
          <table className="w-full text-xs border-collapse">
            <thead className="sticky top-0 bg-red-600 text-white">
              <tr>
                {['NO. HUTANG','TGL. HUTANG','GRAND TOTAL','RETURN','SISA HUTANG','SUDAH DIBAYAR'].map(h =>
            ...
            <tbody>
              ...
              {!loading && filtered.map((h, i) => (
                <tr key={h._id} onClick={() => onSelect(h)}
                  ...
                  <td className="px-2 py-0.5 border-r border-gray-200 font-medium text-blue-700 whitespace-nowrap">{h.refNo}</td>
                  <td className="px-2 py-0.5 border-r border-gray-200 whitespace-nowrap">
                    {h.tanggal ? new Date(h.tanggal).toLocaleDateString('id-ID') : ''}
                  </td>
                  <td className="px-2 py-0.5 border-r border-gray-200 text-right">{fmt(h.grandTotal)}</td>
                  <td className="px-2 py-0.5 border-r border-gray-200 text-right">{h.returnAmount ? fmt(h.returnAmount) : '-'}</td>
                  <td className="px-2 py-0.5 border-r border-gray-200 text-right font-semibold text-red-700">{fmt(h.hutang)}</td>
                  <td className="px-2 py-0.5 text-right">{fmt(h.lunas)}</td>
                </tr>
              ))}
              {!loading && filtered.length === 0 && (
                <tr><td colSpan={6} className="text-center py-6 text-gray-400 italic text-xs">
                  {supplierId ? 'Tidak ada hutang outstanding' : 'Pilih supplier dulu'}
                </td></tr>
              )}
            </tbody>
          </table>
...
export default function BayarHutangPage() {
  // List
  const [list, setList] = useState<BayarHutangDoc[]>([]);
  const [loadingList, setLoadingList] = useState(false);
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [selectedDoc, setSelectedDoc] = useState<BayarHutangDoc | null>(null);
  ...
  const [supplierKodeInput, setSupplierKodeInput] = useState('');
  ...
  const [showHutangPicker, setShowHutangPicker] = useState(false);
  const [hutangPickerQ, setHutangPickerQ] = useState('');
  const [targetRow, setTargetRow] = useState(0);

  // ── Totals ─────────────────────────────────────────────────────────────────

  const totalBayar = items.filter(r => r.transaksiBeliId).reduce((s, r) => s + (r.angsuran || 0), 0);
  ...
  // Update selected doc detail
  useEffect(() => {
    if (!selectedId) { setSelectedDoc(null); return; }
    const doc = list.find(d => d._id === selectedId) || null;
    setSelectedDoc(doc);
  }, [selectedId, list]);
  ...
  function resetForm() {
    ...
    setSupplierId(''); setSupplierKode(''); setSupplierKodeInput('');
    setSupplierNama(''); setSupplierAlamat('');
  ...
  async function handleTambah() {
    ...
    const res = await fetch(`/api/bayar-hutang?action=next-ref&date=${today}`);
  ...
  async function handleEdit() {
    ...
    const res = await fetch(`/api/bayar-hutang/${selectedId}`);
    const doc = await res.json();
    setEditId(doc._id); setRefNo(doc.refNo); setTanggal(toDateInput(doc.tanggal));
    setSupplierId(doc.supplierId || ''); setSupplierKode(doc.supplierKode || '');
    setSupplierKodeInput(doc.supplierKode || ''); setSupplierNama(doc.supplierNama || '');
    ...
    const loadedItems: ItemRow[] = (doc.items || []).map((it: Omit<ItemRow, '_key' | 'noHutangInput'>) => ({
      ...it, _key: Math.random().toString(36).slice(2), noHutangInput: it.noHutang,
    }));
  ...
  async function handleHapus() {
    if (!selectedId) return;
    if (!confirm('Hapus transaksi bayar hutang ini? Hutang supplier akan dikembalikan.')) return;
    await fetch(`/api/bayar-hutang/${selectedId}`, { method: 'DELETE' });
  ...
  function handleSelectSupplier(s: SupplierOption) {
    setSupplierId(s._id); setSupplierKode(s.kode); setSupplierKodeInput(s.kode);
    setSupplierNama(s.nama); setSupplierAlamat(s.alamat);
  ...
  function handleSelectHutang(h: HutangOption) {
    // Check if already added
    if (items.some(r => r.transaksiBeliId === h._id)) {
      setShowHutangPicker(false); return;
    }
    const patch: Partial<ItemRow> = {
      transaksiBeliId: h._id, noHutang: h.refNo, noHutangInput: h.refNo,
      tglHutang: toDateInput(h.tanggal),
      jmlHutang: h.hutang,
    ...
    setShowHutangPicker(false);
  }

  // ── Load all hutang for supplier ────────────────────────────────────────────
  ...
  async function handleLoadAllHutang() {
    ...
    const newRows: ItemRow[] = beliData
      ...
      .map((b) => ({
        ...
        noHutang: b.refNo,
        tglHutang: toDateInput(b.tanggal),
        jmlHutang: b.hutang,
        returnAmount: returnByRef[b.refNo] || 0,
        angsuran: b.hutang,
      }));
    if (newRows.length === 0) { setError('Tidak ada hutang outstanding untuk supplier ini.'); return; }
    setItems(prev => ensureEmptyLast([...prev.filter(r => r.transaksiBeliId), ...newRows]));
  }

  // ── Item update ────────────────────────────────────────────────────────────

  function updateAngsuran(idx: number, val: number) {
    setItems(prev => prev.map((r, i) => i !== idx ? r : { ...r, angsuran: Math.max(0, Math.min(val, r.jmlHutang)) }));
  }
  ...
  async function handleSave() {
    setError('');
    const validItems = items.filter(r => r.transaksiBeliId && r.angsuran > 0);
    if (validItems.length === 0) { setError('Minimal 1 hutang harus diisi.'); return; }
    if (!kasId) { setError('Pilih kas pembayaran.'); return; }
    setSaving(true);
    try {
      const body = {
        refNo, tanggal, supplierId, supplierKode, supplierNama, supplierAlamat,
        kasId, kasKode, kasNama, keterangan, operator,
        totalBayar,
        items: validItems.map(({ _key, noHutangInput, ...r }) => r),
      };
      const isEdit = !!editId;
      const url = isEdit ? `/api/bayar-hutang/${editId}` : '/api/bayar-hutang';
  ...
  return (
    <>
              ...
              {selectedDoc ? (
                <>
                  ...
                  <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-xs mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="font-semibold w-24 shrink-0">SUPPLIER</span>
                      <span className="border bg-white px-2 py-0.5 rounded flex-1">{selectedDoc.supplierNama}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="font-semibold w-24 shrink-0">JUMLAH</span>
                      <span className="border bg-white px-2 py-0.5 rounded flex-1 text-right font-bold text-red-700">{fmt(selectedDoc.totalBayar)}</span>
                    </div>
                    <div className="flex items-center gap-2">
                  ...
                  <div className="flex-1 overflow-auto">
                    <table className="w-full text-xs border-collapse">
                      <thead className="sticky top-0 bg-blue-600 text-white">
                        <tr>
                          {['NO. HUTANG','TGL. HUTANG','JML HUTANG','JML BAYAR'].map(h =>
                      ...
                      <tbody>
                        {selectedDoc.items.map((it, i) => (
                          <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-blue-50/40'}>
                            <td className="px-2 py-0.5 border border-gray-200 font-medium">{it.noHutang}</td>
                            <td className="px-2 py-0.5 border border-gray-200 whitespace-nowrap">
                              {it.tglHutang ? new Date(it.tglHutang).toLocaleDateString('id-ID') : ''}
                            </td>
                            <td className="px-2 py-0.5 border border-gray-200 text-right">{fmt(it.jmlHutang)}</td>
                            <td className="px-2 py-0.5 border border-gray-200 text-right font-bold text-red-700">{fmt(it.angsuran)}</td>
                          </tr>
                        ))}
          ...
          <div className="bg-blue-100 border-b-2 border-blue-300 px-4 py-2 shrink-0">
            <div className="flex gap-6 items-start">
              ...
              <div className="ml-auto flex flex-col items-end justify-center">
                ...
                <div className="text-xs text-gray-500 mt-1 font-semibold">TOTAL PEMBAYARAN</div>
              </div>
            </div>
          </div>
            ...
            <table className="border-collapse text-xs w-full">
              <thead className="sticky top-0 z-10 bg-blue-700 text-white">
                <tr>
                  ...
                  <th className="px-2 py-1 border border-blue-600 text-left" style={{ minWidth: 160 }}>NO. HUTANG</th>
                  <th className="px-2 py-1 border border-blue-600 text-left w-28">TGL. HUTANG</th>
                  <th className="px-2 py-1 border border-blue-600 text-right w-36">JML. HUTANG</th>
                  <th className="px-2 py-1 border border-blue-600 text-right w-32">RETURN</th>
                  <th className="px-2 py-1 border border-blue-600 text-right w-40">ANGSURAN HUTANG</th>
              ...
              <tbody>
                {items.map((row, idx) => {
                  ...
                  const isEmpty = !row.transaksiBeliId;
                  return (
                    <tr key={row._key} onClick={() => setActiveRow(idx)}
                      ...
                      <td className="border-r border-gray-200 p-0">
                        <input value={row.noHutangInput} readOnly
                          placeholder={isActive ? 'Enter untuk cari hutang...' : ''}
                          onClick={() => { setActiveRow(idx); setTargetRow(idx); setHutangPickerQ(row.noHutangInput); setShowHutangPicker(true); }}
                          onKeyDown={e => { if (e.key === 'Enter') { setTargetRow(idx); setHutangPickerQ(row.noHutangInput); setShowHutangPicker(true); } }}
                      ...
                      <td className="px-2 py-0.5 border-r border-gray-200 text-xs whitespace-nowrap">
                        {row.tglHutang ? new Date(row.tglHutang).toLocaleDateString('id-ID') : ''}
                      </td>

                      {/* JML. HUTANG */}
                      <td className="px-2 py-0.5 border-r border-gray-200 text-right text-xs">
                        {row.jmlHutang ? fmt(row.jmlHutang) : ''}
                      </td>

                      {/* RETURN */}
                      <td className="px-2 py-0.5 border-r border-gray-200 text-right text-xs">
                        {row.returnAmount ? fmt(row.returnAmount) : (row.transaksiBeliId ? '' : '')}
                      </td>
      ...
      {showHutangPicker && (
        <HutangPicker supplierId={supplierId} q={hutangPickerQ}
          onSelect={handleSelectHutang} onClose={() => setShowHutangPicker(false)} />
      )}
    </>
  );
}
